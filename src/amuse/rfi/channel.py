...
class AsyncRequestsPool(object):
    ...
    def add_request(
        ...
        if async_request in self.registered_requests:
            raise Exception(
                "Request is already registered, cannot register a request more"
                " than once")
    ...
    def wait(self):
        ...
        while self.requests_and_handlers:
            ...
            if requests:
                ...
                request_and_handler.async_request.waits_for().waitone()
...
class MPIMessage(AbstractMessage):
    ...
    def receive_strings(self, comm, total):
        if total > 0:
            ...
            for size in sizes:
                strings.append(
                    data_bytes[begin:begin + size].tostring().decode('latin_1')
                )
                begin = begin + size + 1
...
class AbstractMessageChannel(OptionalAttributes):
    ...
    @classmethod
    def GDB(
            cls, full_name_of_the_worker, channel,
            interpreter_executable=None
    ):
        arguments = [
            '-hold', '-display', os.environ['DISPLAY'], '-e', 'gdb', '--args'
        ]
    ...
    def DDD(
        ...
        if os.name == 'nt':
            arguments = [
                full_name_of_the_worker, "--args", full_name_of_the_worker
            ]
            command = channel.adg_exe
            return command, arguments
        else:
            arguments = [
                '-display', os.environ['DISPLAY'], '-e', 'ddd', '--args'
            ]
    ...
    @classmethod
    def VALGRIND(
            cls, full_name_of_the_worker, channel, interpreter_executable=None
    ):
    ...
    @classmethod
    def REDIRECT(
            cls, full_name_of_the_worker, stdoutname, stderrname, command=None,
    ...
    def check_if_worker_is_up_to_date(self, object):
        ...
        modificationtime_of_worker = os.stat(
            name_of_the_compiled_file).st_mtime
            ...
            if hasattr(value, 'crc32'):
                ...
                if not is_up_to_date:
                    raise exceptions.CodeException(
                        "The worker code of the '{0}' interface class is not"
                        " up to date. Please do a 'make clean; make' in the"
                        " root directory.".format(type(object).__name__)
                    )
    ...
    def get_full_name_of_the_worker(self, type):
        ...
        exe_name = (
            self.worker_code_prefix + self.name_of_the_worker
...
class MpiChannel(AbstractMessageChannel):
    ...
    def __init__(
        ...
        if self.check_mpi:
            if not is_mpd_running():
                raise exceptions.CodeException(
                    "The mpd daemon is not running, please make sure it is"
                    " started before starting this code"
                )

        if self._mpi_is_broken_after_possible_code_crash:
            raise exceptions.CodeException(
                "Another code has crashed, cannot spawn a new code, please"
                " stop the script and retry"
            )
    ...
    @option(
        choices=AbstractMessageChannel.DEBUGGERS.keys(), sections=("channel",)
    )
    def debugger(self):
    ...
    def start(self):
        ...
        if self.debugger_method is not None:
            command, arguments = self.debugger_method(
                self.full_name_of_the_worker, self,
    ...
    def nonblocking_recv_message(
        ...
        def handle_result(function):
            ...
            if has_units:
                return (
                    message.to_result(handle_as_array), message.encoded_units
                )
            else:
                return message.to_result(handle_as_array)
    ...
    def get_info_from_slurm(cls, number_of_workers):
        ...
        host = ','.join(hostnames)
        print "HOST: %s %s %s" % (
            host, cls._scheduler_index, os.environ['SLURM_TASKS_PER_NODE']
        )
...
m.run_mpi_channel('{2}')"""
    ...
    def send_message(
        ...
        self._send(
            self.client_socket,
            ('send_message', (call_id, function_id, dtype_to_arguments),)
        )
        result = self._recv(self.client_socket)
        return result
...
class SocketMessage(AbstractMessage):
    ...
    def receive(self, socket):
        ...
        # logger.debug(
        #     "receiving message with flags %s and header %s", flags, header)
    ...
    def send(self, socket):
        ...
        # logger.debug(
        #     "sending message with flags %s and header %s", flags, header)
...
class SocketChannel(AbstractMessageChannel):
    ...
    def __init__(
        ...
        if (
            ...
            raise exceptions.CodeException(
                "can only run codes on local machine using SocketChannel, not"
                " on %s", self.hostname
            )
    ...
    def start(self):
        ...
        logger.debug(
            "starting socket worker process, listening for worker connection"
            " on %s",
        ...
        if self.debugger_method is not None:
            ...
            if (
                ...
                command, arguments = self.REDIRECT(
                    self.full_name_of_the_worker,
                    self.redirect_stdout_file, self.redirect_stderr_file,
                    command=self.python_exe_for_redirection,
                    interpreter_executable=self.interpreter_executable
                )
        ...
        logger.debug(
            ...
            " environment '%s'",
            command, arguments, os.environ
        )
        self.process = Popen(
            arguments, executable=command, stdin=PIPE, stdout=None,
...
class DistributedChannel(AbstractMessageChannel):
    ...
    def __init__(
            self, name_of_the_worker, legacy_interface_type=None,
            interpreter_executable=None, distributed_instance=None,
            dynamic_python_code=False, **options
    ):
        AbstractMessageChannel.__init__(self, **options)
        ...
        logger.debug(
            ...
            self.number_of_workers, self.number_of_threads, self.label
        )
        ...
        logger.debug(
            "full name of the worker is %s", self.full_name_of_the_worker
        )
    ...
    def start(self):
        ...
        arguments = {
            'string': [
                self.executable, self.redirect_stdout_file,
                self.redirect_stderr_file, self.label, self.worker_dir
            ],
            'int32': [
                self.number_of_workers, self.number_of_threads
            ],
            'bool': [self.dynamic_python_code]
        }

        message = SocketMessage(
            call_id=1, function_id=10101010, call_count=1,
        ...
        logger.info(
            "waiting for worker %s to be initialized", self.name_of_the_worker
        )
        ...
        if result.error:
            ...
            raise exceptions.CodeException(
                ...
                + self.name_of_the_worker + ": " + result.strings[0]
            )
    ...
    def send_message(
        ...
        logger.debug(
            "sending message for call id %d, function %d, length %d",
            call_id, function_id, call_count
        )
        ...
        if call_count > self.max_message_length:
            ...
            message = SocketMessage(
                call_id, function_id, call_count, dtype_to_arguments, False,
                False
            )
            message.send(self.socket)
...
class LocalChannel(AbstractMessageChannel):
    ...
    def recv_message(
        ...
        output_message = LocalMessage(
            call_id, function_id, self.message.call_count)
