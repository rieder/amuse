...
def check_energy_conservation(system, i_step, time, n_steps):
    ...
    print(
        f"Step {i_step} of {n_steps}, t={time.as_quantity_in(units.yr)}: "
        f"U={U:.2e}, Q={Q:.2e}, K={K:.2e} {unit}"
    )
...
def local_relax(
    ...
    """
    Relax a set of SPH particles by evolving it with a hydrodynamics code,
    while imposing critical damping on the particle velocities.

    :argument gas_particles:  The set of SPH particles
    :argument hydro:          The hydrodynamics code
    :argument gravity_field   Background gravitational field, must support
    get_gravity_at_point
    :argument monitor_func    For monitoring progress each step. User-defined
    function or "energy"
    ...
    velocity_damp_factor = (
        1.0 - (2.0 * numpy.pi * t_end_in_t_dyn) / n_steps
    )  # Critical damping
    ...
    if gravity_field is None:
        ...
        system = Bridge(
            timestep=(t_end / n_steps).as_quantity_in(units.yr), **bridge_options
        )
...
def generate_initial_conditions(
    ...
    if use_fractal:
        ...
        stars = new_plummer_model(
            number_of_stars, convert_nbody=converter, do_scale=False
        )
    stars.mass = masses
    ...
    stars.velocity *= numpy.sqrt(
        virial_ratio
        * converter.to_si(0.5 | nbody_system.energy)
    ...
    filename = "YSC_{0}_stars{1}_gas{2}k_".format(
        "fractal" if use_fractal else "plummer",
        number_of_stars,
...
def plot_hydro_and_stars(hydro, stars):
    ...
    cbar = ax.colorbar(cax, orientation="vertical", fraction=0.045)
    cbar.set_ticks([1.0, 1.5, 2.0, 4.0, 4.5, 5.0])
    cbar.set_label(r"$\log_{10} (\mathrm{n}) \,\,\, [\mathrm{amu/cm}^3]$", rotation=270)
