...
class UnitlessArgs:
    ...
    def strip(self, *args, **kwargs):
        if self.current_plot is native_plot.gca():
            args = [
                arg.as_quantity_in(unit) 
                if quantities.is_quantity(arg)
                else arg for arg, unit in map(
                    lambda *x: tuple(x), args, self.arg_units
                )
            ]
        self.clear()
...
def smart_length_units_for_vector_quantity(quantity):
    length_units = [units.Mpc, units.kpc,
                    units.parsec, units.AU, units.RSun, units.km]
...
def sph_particles_plot(particles, u_range=None, min_size=100, max_size=10000,
    ...
    a set of SPH particles. The particles must have the following attributes
    defined:
    position, u, h_smooth.
    ...
    :argument gd_particles: non-SPH particles can be indicated with white
    circles
    ...
    clipped_log_u = numpy.minimum(numpy.ones_like(
        log_u), numpy.maximum(numpy.zeros_like(log_u), log_u))
    ...
    n_pixels = (
        native_plot.gcf().get_dpi()
        * native_plot.gcf().get_size_inches()
    )
    ...
    if view:
        ...
        current_axes.set_xlim(
            view[0].value_in(length_unit),
            view[1].value_in(length_unit), emit=True, auto=False)
        current_axes.set_ylim(
            view[2].value_in(length_unit),
            view[3].value_in(length_unit), emit=True, auto=False)
        phys_to_pix2 = n_pixels[0]*n_pixels[1] / \
            ((view[1]-view[0])**2 + (view[3]-view[2])**2)
    else:
    ...
    sizes = numpy.minimum(numpy.maximum(
        (h_smooths**2 * phys_to_pix2), min_size), max_size)
...
def convert_particles_to_pynbody_data(
        particles, length_unit=units.kpc, pynbody_unit="kpc"):
    if not HAS_PYNBODY:
    ...
    if hasattr(particles, "mass"):
        pynbody_data['mass'] = SimArray(
            particles.mass.value_in(units.MSun), "Msol")
    if hasattr(particles, "position"):
        pynbody_data['x'] = SimArray(
            particles.x.value_in(length_unit), pynbody_unit)
        pynbody_data['y'] = SimArray(
            particles.y.value_in(length_unit), pynbody_unit)
        pynbody_data['z'] = SimArray(
            particles.z.value_in(length_unit), pynbody_unit)
    if hasattr(particles, "velocity"):
        pynbody_data['vx'] = SimArray(
            particles.vx.value_in(units.km / units.s), "km s^-1")
        pynbody_data['vy'] = SimArray(
            particles.vy.value_in(units.km / units.s), "km s^-1")
        pynbody_data['vz'] = SimArray(
            particles.vz.value_in(units.km / units.s), "km s^-1")
    if hasattr(particles, "h_smooth"):
        pynbody_data['smooth'] = SimArray(
            particles.h_smooth.value_in(length_unit), pynbody_unit)
    if hasattr(particles, "rho"):
        pynbody_data['rho'] = SimArray(
            particles.rho.value_in(units.g / units.cm**3), "g cm^-3")
...
def mu(X=None, Y=0.25, Z=0.02, x_ion=0.1):
    """
    Compute the mean molecular weight in kg (the average weight of particles in
    a gas) X, Y, and Z are the mass fractions of Hydrogen, of Helium, and of
    metals, respectively.
...
def _smart_length_units_for_pynbody_data(length):
    length_units = [(units.Gpc, "Gpc"), (units.Mpc, "Mpc"), (units.kpc, "kpc"),
                    (units.parsec, "pc"), (units.AU,
                                           "au"), (1.0e9*units.m, "1.0e9 m"),
...
def pynbody_column_density_plot(particles, width=None, qty='rho', units=None,
    ...
    pyndata = convert_particles_to_pynbody_data(
        particles, length_unit, pynbody_unit)
    ...
    result = function(pyndata, width=width.value_in(
        length_unit), qty=qty, units=units, **kwargs)
...
def effective_iso_potential_plot(gravity_code,
                                 ...
                                 projection3D=False):
    """
    Create a contour plot of the effective potential of particles in a gravity
    code.
    The code needs to support 'get_potential_at_point' only, so it can also be
    an instance of Bridge.
    ...
    potential -= omega**2 * \
        ((x-center_of_rotation[0])**2 + (y-center_of_rotation[1])**2) / 2.0

    if projection3D:
        from matplotlib import cm
        ax = native_plot.gca(projection='3d')
        Z = potential.number.reshape(resolution[::-1])
        levels = set_contour_levels(
            potential, number_of_contours, fraction_screen_filled, quadratic_contour_levels)
        Z = numpy.maximum(Z, levels[0])
        ax.plot_surface(
            x_num, y_num, Z, rstride=1, cstride=1, cmap=cm.spectral,
            linewidth=0, antialiased=False, vmin=levels[0], vmax=3*levels[-1]-2
            * levels[0])
    ...
    levels = set_contour_levels(
        potential, number_of_contours, fraction_screen_filled,
        quadratic_contour_levels
    )
    CS = native_plot.contour(x_num, y_num, potential.number.reshape(
        resolution[::-1]), levels, **contour_kwargs)

    if omega2 is None:
        return potential

    potential2 = potential - omega2**2 * \
        ((x-center_of_rotation2[0])**2 + (y-center_of_rotation2[1])**2) / 2.0
    # ~levels = set_contour_levels(potential, number_of_contours2, fraction_screen_filled2, quadratic_contour_levels2)
    levels = set_contour_levels(
        potential2, number_of_contours, fraction_screen_filled2,
        quadratic_contour_levels)
    CS = native_plot.contour(
        x_num, y_num, potential2.number.reshape(
            resolution[::-1]), levels, **contour_kwargs)
...
def set_contour_levels(potential, number_of_contours, fraction_screen_filled, quadratic_contour_levels):
    ...
    V_min = potential.sorted().number[int(
        len(potential)*(1-fraction_screen_filled))]
