...
int set_hermes_hill_switch_factor(double value, int code_index){
    if(code_index < 0 || code_index >= (signed) codes.size()){
        return -10;
    }
    if(!codes[code_index].code) {
    ...
    if(code->integrator==reb_simulation::REB_INTEGRATOR_HERMES) {
        code->ri_hermes.hill_switch_factor = value;
    }
    else {
        // Current code is not Hermes, so not setting this value
        //FIXME Which error code should be used here?
...
int get_hermes_hill_switch_factor(int code_index, double * value){
    ...
    if(code->integrator==reb_simulation::REB_INTEGRATOR_HERMES) {
        *value = code->ri_hermes.hill_switch_factor;
    }
    else {
        return -1;
    }
