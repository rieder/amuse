...
class HydrodynamicsInterface(common.CommonCodeInterface):
    ...
    def get_index_range_inclusive(self):
        """
        Returns the min and max values of indices in each direction. The range
        is inclusive, the min index and max index both exist and can be
        queried.  The total number of cells in one direction is max - min + 1.

        The returntype is a list of 3 tuples, each tuple contains the minimum
        and maximum value in the index range.

        For C/C++ codes the returned values will usually be: ((0, nmeshx-1),
        (0, nmeshy-1), (0, nmeshz-1))


        For Fortran codes the returned values will usually be: ((1, nmeshx),
        (1, nmeshy), (1, nmeshz))
        """
        pass
    ...
    def get_position_of_index():
        """
        Retrieves the x, y and z position of the center of the cell with
        coordinates i, j, k in the grid specified by the index_of_grid
        """
