...
class Hydro:
    def __init__(self, hydro_code, particles, stars):
        ...
        if hydro_code not in [Fi, Gadget2]:
            raise Exception(
                "unsupported Hydro code: %s" % (hydro_code.__name__)
            )
        ...
        N = len(particles)
        dt = (
            0.004 * numpy.pi * numpy.power(eps, 1.5)
        ...
        self.star_attributes = [
            'name', 'birth_age', 'angular_momentum', 'mass', 'radius', 'x',
            'y', 'z', 'vx', 'vy', 'vz', 'Lx', 'Ly', 'Lz'
        ]

        if hydro_code is Fi:
            self.code = hydro_code(
                self.converter, mode="openmp", redirection="file"
            )
            ...
            """
            isothermal_flag:
            When True then we have to do our own adiabatic cooling (and Gamma
            has to be 1.0)
            When False then we dont do the adjabatic cooling and Fi is changing
            u
            """
        ...
        if hydro_code is Gadget2:
            ...
            self.code.parameters.begin_time = 0.0 | units.Myr
            self.code.parameters.time_max = (
                dt*2**int(numpy.log2(4*(10 | units.Myr)/dt))
            )
        ...
        self.channel_to_stars = self.code.dm_particles.new_channel_to(
            self.star_particles
        )
        self.channel_from_stars = self.star_particles.new_channel_to(
    ...
    def evolve_model(self, model_time):
        ...
        while density_limit_detection.is_set():
            ...
            print(
                "end N=", len(self.star_particles), len(self.code.dm_particles)
            )
        ...
        # self.code.evolve_model(model_time)
        print(
            "final N=", len(self.star_particles), len(self.code.dm_particles)
        )
        print(
            "final Ngas=", len(self.gas_particles),
    ...
    def resolve_sinks(self):
        ...
        print("new sinks...")
        # had to make some changes to prevent double adding particles
        if candidate_stars:
...
def merge_two_stars(bodies, particles_in_encounter):
    ...
    print(
        "Two stars (M=", particles_in_encounter.mass, ") collided at d=",
        com_pos.length()
    )
